...
struct DKEntries {
    ...
    static func edit() -> String? {
        ...
        do {
            ...
            print("Failed to edit DKEntries files")
        }
        ...
        for (index, row) in rows.enumerated() {
            if index >= 1 && index <= Lineups.count {
                ...
                if let points = lineups.keys.max() { // points is the dictionary key. This will be used later to delete lineup
                    ...
                    if let count = lineups.values.first?.count {
                        for n in 4...count + 4 - 1 {
                            guard let player = Player.info[getID(index: n - 4, points)] else { continue }
                            info[n] = "\(player.name) (\(player.id))"
                        }
                    }
